...
(function() {
  ...
  // Close mobile menu when clicking on a link (but not dropdown toggle or dropdown menu items)
  const navLinks = document.querySelectorAll('.nav-menu a:not(.dropdown-toggle):not(.dropdown-menu a)');
  ...
  // Don't close mobile menu when clicking dropdown menu items - keep menu open so users can access both Home 1 and Home 2
  ...
  dropdownMenuLinks.forEach(function(link) {
    link.addEventListener('click', function(e) {
      ...
      // This allows users to access both Home 1 and Home 2 without reopening menu
      e.stopPropagation();
      // Don't close the menu - let users navigate between Home 1 and Home 2
    });
  });

  // ========== Dropdown Menu Toggle ==========
  const dropdownToggles = document.querySelectorAll('.nav-menu li.dropdown > a');
  dropdownToggles.forEach(function(toggle) {
    toggle.addEventListener('click', function(e) {
      // Only handle on mobile/tablet (including iPad Pro at 1024px)
  ...
  // Close dropdown when clicking outside (mobile/tablet) but keep mobile menu open
  document.addEventListener('click', function(e) {
    if (window.innerWidth <= 1024) {
      // Don't close dropdown if clicking on dropdown menu items
      if (e.target.closest('.dropdown-menu')) {
        return;
      }
      
      // Don't close dropdown if clicking on dropdown toggle
      if (e.target.closest('.nav-menu li.dropdown > a')) {
        return;
      }
